...
date: 2025-09-16
tags: [NBA, Basketabll, Python, Web Scraping]
...
<img src="/images/nba_top_game_finder.png" alt="Home page of Inpredictable's Top Game Finder, shows a table of 25 games sorted by highest excitement">
...
<img src="/images/nba_top_game_finder_date_example.png" alt="Excitement and tension data for the 12 games on November 1st, 2023">

and the URL is: https://stats.inpredictable.com/nba/topGame.php?sort=ei&season=ALL&team=ALL&po=0&frdt=2023-11-01&todt=2023-11-01. 

Two more observations: 

* `po=0` is  the setting which excludes playoff games
* To filter to a specific date, we need to set `frdt` and `todt` to the same date, and use the YYYY-MM-DD format.
</p>
...
<p> Using our November 1st, 2023 day as an example, we would be able to read information from the page by sending a request. Since we know that the results are stored in a table, we can use `pd.read_html()` to
read the HTML table into a dataframe. `StringIO(r.text)` in this instance transforms our webpage, stored as `r`, into a file-like object, so that it is compatible with the `read_html` command. `read_html` also 
...
`game_table['Excitement']` and `game_table['Tension']`, respectively. `list(game_table['Excitement'].values)` gives us the excitements as a list.
</p>
...
url = f'https://stats.inpredictable.com/nba/topGame.php?sort=ei&season=ALL&team=ALL&po=0&frdt=2023-11-01&todt=2023-11-01'
r = requests.get(url)
